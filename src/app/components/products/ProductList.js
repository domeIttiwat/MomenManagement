  ...
  if (viewMode === 'card') {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
        {products.map(p => <ProductCard key={p.id} product={p} showCost={showCost} onClick={() => onSelectProduct(p)} />)}
      </div>
    );
  }
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1000px]">
          ...
          <tbody className="divide-y divide-gray-50">
            {products.map(p => <ProductListItem key={p.id} product={p} showCost={showCost} onClick={() => onSelectProduct(p)} />)}
          </tbody>
        </table>
      </div>
    </div>
