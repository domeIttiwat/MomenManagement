...
const OrderList = ({ orders, showProfit, onSelect, tagPropsFor }) => {
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1120px] text-left">
          ...
          <tbody>
            {orders.map(o => <OrderListItem key={o.id} order={o} showProfit={showProfit} onClick={() => onSelect(o)} {...(tagPropsFor ? tagPropsFor(o.id) : {})} />)}
          </tbody>
        </table>
      </div>
    </div>
