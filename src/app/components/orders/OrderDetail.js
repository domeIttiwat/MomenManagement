...
import { ArrowLeft, Edit, Trash2, Printer, FileText, User, Package, Clock, MapPin, Phone, CreditCard, DollarSign, X, Eye, EyeOff, Banknote, Landmark, MessageCircle, Facebook, Instagram, ExternalLink } from 'lucide-react';
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/80 backdrop-blur-md p-4 rounded-2xl shadow-sm border border-gray-100 sticky top-2 z-20">
        ...
        <div className="flex flex-wrap gap-2">
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold transition-all text-sm border ${showProfit ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-white text-gray-500 border-gray-200'}`}
          >
            {showProfit ? <Eye size={18}/> : <EyeOff size={18}/>} {showProfit ? 'ซ่อนกำไร' : 'แสดงกำไร'}
          </button>
          <button 
            onClick={() => setShowBill(true)} 
            className="flex items-center gap-2 px-4 py-2.5 bg-indigo-50 text-indigo-700 border border-indigo-100 hover:bg-indigo-100 rounded-xl font-semibold transition-all text-sm shadow-sm"
          >
            <Printer size={18}/> พิมพ์/ดูเอกสาร
          </button>
          <button 
            onClick={onEdit} 
            className="flex items-center gap-2 px-5 py-2.5 bg-gray-900 text-white rounded-xl hover:bg-black font-medium text-sm shadow-lg shadow-gray-200 transition-all active:scale-95"
          >
            <Edit size={18}/> แก้ไข
          </button>
          <button 
            onClick={onDelete} 
            className="flex items-center gap-2 px-3 py-2.5 bg-white text-red-500 border border-gray-200 rounded-xl hover:bg-red-50 hover:border-red-100 font-medium text-sm transition-all active:scale-95"
          >
            <Trash2 size={18}/>
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-8 opacity-[0.03] transform group-hover:scale-110 transition-transform duration-700 pointer-events-none">
              <FileText size={200} />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-100 relative z-10">
              ...
              <div 
                onClick={() => onViewCustomer && onViewCustomer(order.customer_id)}
                className="flex gap-4 p-3 -ml-3 rounded-xl hover:bg-indigo-50/50 cursor-pointer transition-colors group/customer border border-transparent hover:border-indigo-100"
                title="คลิกเพื่อดูข้อมูลลูกค้า"
              >
                <div className="w-12 h-12 bg-indigo-50 rounded-full flex items-center justify-center text-indigo-600 shrink-0 group-hover/customer:bg-white group-hover/customer:shadow-sm transition-all">
                  <User size={24} />
                </div>
                <div>
                  <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wide mb-1 flex items-center gap-2">
                    ลูกค้า 
                    <span className="text-[10px] text-indigo-500 bg-indigo-50 px-1.5 py-0.5 rounded-full flex items-center gap-1 opacity-0 group-hover/customer:opacity-100 transition-opacity">
                      <ExternalLink size={10}/> ดูประวัติ
                    </span>
                  </h3>
                  <p className="font-bold text-gray-800 text-lg group-hover/customer:text-indigo-700 transition-colors">
                  ...
                  <div className="flex-1 bg-gray-50 p-3 rounded-xl border border-gray-100">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <div className="text-right">
                        <span className="font-bold text-gray-900 block">฿{pay.amount.toLocaleString()}</span>
                        {pay.fee_amount > 0 && <span className="text-[10px] text-red-400 block">+ค่าธรรมเนียม {Number(pay.fee_amount).toLocaleString()}</span>}
                      </div>
                    </div>
                  </div>
                </div>
