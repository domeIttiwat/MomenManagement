...
import { Upload, X, Star, ImagePlus, Eye, Loader2 } from 'lucide-react';
    ...
    <div className="space-y-4">
      {previewImage && (
        <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={() => setPreviewImage(null)}>
          <img src={previewImage} className="max-w-full max-h-full rounded-lg shadow-2xl object-contain" />
          ...
          <div key={idx} className="relative aspect-square rounded-xl border border-gray-200 overflow-hidden group bg-gray-50 shadow-sm">
            ...
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all flex flex-col items-center justify-center gap-2 backdrop-blur-[1px]">
               ...
               <div className="flex gap-2">
                 <button type="button" onClick={() => setThumbnail(idx)} className={`p-1.5 rounded-full transition-all ${idx === 0 ? 'bg-yellow-400 text-white' : 'bg-white/20 text-white hover:bg-yellow-400'}`} title="ตั้งเป็นรูปหลัก"><Star size={14} fill={idx === 0 ? "currentColor" : "none"}/></button>
                 <button type="button" onClick={() => removeImage(idx)} className="p-1.5 bg-white/20 text-white hover:bg-red-500 rounded-full transition-all" title="ลบรูป"><X size={14}/></button>
               </div>
            </div>
            {idx === 0 && <div className="absolute bottom-0 w-full bg-indigo-600/90 text-white text-[10px] text-center py-0.5 font-bold tracking-wider">COVER</div>}
          </div>
        ))}
