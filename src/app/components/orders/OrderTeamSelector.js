            ...
            <div key={i} className="flex items-center gap-3 bg-white border border-gray-200 rounded-xl p-3 shadow-sm group">
              ...
              <div className="flex-1 min-w-0 grid grid-cols-1 sm:grid-cols-2 gap-2 items-center">
                ...
                <div className="flex flex-col gap-1">
                  <select 
                    className={`text-xs border rounded-lg px-2 py-1.5 focus:outline-none focus:border-indigo-500 w-full font-medium cursor-pointer ${selectValue === 'อื่นๆ' ? 'bg-white border-indigo-300 text-indigo-700' : 'bg-gray-50 border-gray-200 text-gray-700'}`}
                    value={selectValue}
                    onChange={e => {
                        const val = e.target.value;
                        updateRole(i, val === 'อื่นๆ' ? '' : val);
                    }}
                  >
              ...
              <button type="button" onClick={() => removeAssignee(i)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><X size={18}/></button>
            </div>
          );
        })}
