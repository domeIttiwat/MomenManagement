...
import { MapPin, Package, Trophy, Bike, Wrench } from 'lucide-react';

const TopRankings = ({ topScooters = [], topAccessories = [], topLocations = [] }) => {
  const [productTab, setProductTab] = useState('scooters'); // 'scooters' | 'accessories'

  const currentProducts = productTab === 'scooters' ? topScooters : topAccessories;
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 h-full flex flex-col">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
          ...
          <div className="flex bg-gray-100 p-1 rounded-xl shrink-0">
             <button 
               onClick={() => setProductTab('scooters')}
               className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${productTab === 'scooters' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
             >
               <Bike size={14}/> สกู๊ตเตอร์
             </button>
             <button 
               onClick={() => setProductTab('accessories')}
               className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${productTab === 'accessories' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
             >
               <Wrench size={14}/> ชุดแต่ง/อะไหล่
             </button>
          </div>
        </div>
        ...
        <div className="space-y-3 flex-1 overflow-y-auto max-h-[400px] pr-2">
          ...
          {currentProducts.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-gray-400 py-10">
              ...
              <p className="text-sm">ไม่มีข้อมูลการขายในหมวดนี้</p>
            </div>
          )}
        </div>
      </div>
