...
import { Plus, Trash2, Calendar, FileText, Clock, Paperclip, X, Edit } from 'lucide-react';
import ImageUploader from '../orders/ImageUploader';

const ServiceUpdateManager = ({ updates = [], onChange }) => {
  const [isAdding, setIsAdding] = useState(false);
  const [editingIndex, setEditingIndex] = useState(-1); // เก็บ index ที่กำลังแก้ไข (-1 คือไม่ได้แก้)
  ...
  const removeUpdate = (idx) => {
    ...
    // ถ้าลบตัวที่กำลังแก้ ให้ยกเลิกการแก้
    ...
    <div className="space-y-4">
      ...
      {isAdding ? (
        <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 animate-in fade-in slide-in-from-top-2 space-y-3">
           ...
           <div>
              <label className="text-[10px] text-gray-500 font-bold mb-1 block">รายละเอียดการทำงาน</label>
              <textarea 
                ...
                placeholder="เช่น ถอดเช็คช่วงล่าง, รออะไหล่..."
        ...
        >
          <Plus size={18}/> เพิ่มการอัปเดตงาน
        </button>
      )}
    </div>
  );
...
export default ServiceUpdateManager;