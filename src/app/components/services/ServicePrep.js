...
const ServicePrep = ({ service, onItemsChange, openSignal }) => {
  ...
  const deductStock = async (it) => {
    ...
    const { data: tx } = await supabase.from('stock_transactions').insert([{
      ...
      quantity: qty,
      note: `เบิกเตรียมของ: ${it.title} (${service.service_number})`,
      reference_type: 'service_prep',
  ...
  const returnStock = async (it) => {
    ...
    await createStockLot({
      ...
      sourceType: 'return',
      note: `คืนของจากเตรียมของ: ${it.title} (${service.service_number})`,
      profileId: profile?.id || null,
      syncSummary: true,
    });
  ...
  const deleteItem = async (it) => {
    ...
    await supabase.from('service_prep_items').delete().eq('id', it.id);
  };
