...
const ServiceMain = ({ onViewCustomer }) => {
  ...
  if (view === 'detail') return <ServiceDetail service={selectedService} onBack={() => { setView('list'); fetchServices(true); }} onEdit={() => setView('form')} onDelete={() => handleDelete(selectedService.id)} showProfit={showProfit} setShowProfit={setShowProfit} onViewCustomer={onViewCustomer} />;
  if (view === 'log') return (
